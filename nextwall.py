...
from easy_shortcut import shortcut
...
root.config(bg='#7063ff')
# language
with open("config.json", "r", encoding="utf-8") as file:
	config = json.load(file)
if config['language']['current'] == "eng":
	lang = config['language']['eng']
elif config['language']['current'] == "rus":
	lang = config['language']['rus']
elif config['language']['current'] == "uzb":
	lang = config['language']['uzb']
# tray
...
class SysTrayIcon(object):
    ...
    def __init__(self,icon,hover_text,menu_options,on_quit=None,default_menu_index=None,window_class_name=None,):
        ...
        menu_options = menu_options + ((lang[5], None, self.QUIT),)
...
files = files.files
# files & wallpaper
def select_files(sysTrayIcon):
    global wps
    wps = askopenfilenames(title=lang[0], filetypes=((lang[7], "*.jpg *.png"),(lang[8], "*.jpg"),(lang[9], "*.png"),(lang[10], "*.*")))
    if wps is None:
        wps = "files = ()"
    with open("files.py", "w", encoding="utf-8") as file2:
        file2.write(f"files = {wps}")
def next_wall(sysTrayIcon):
    w = int(config['w'])
    if w >= len(files):
        w = 0
    ctypes.windll.user32.SystemParametersInfoW(20, 0, files[w], 0)
    w += 1
    config['w'] = str(w)
    with open("config.json", "w", encoding="utf-8") as file3:
        json.dump(config, file3, indent=4)
def per_wall(sysTrayIcon):
    w = int(config['w'])
    if w <= 0:
        w = 0
    ctypes.windll.user32.SystemParametersInfoW(20, 0, files[w], 0)
...
def eng(sysTrayIcon):
    ...
    with open("config.json", "w", encoding="utf-8") as file4:
        json.dump(config, file4, indent=4)
    showinfo(message=lang[14])
def rus(sysTrayIcon):
    config['language']['current'] = "rus"
    with open("config.json", "w", encoding="utf-8") as file5:
        json.dump(config, file5, indent=4)
    showinfo(message=lang[14])
def uzb(sysTrayIcon):
    config['language']['current'] = "uzb"
    with open("config.json", "w", encoding="utf-8") as file6:
        json.dump(config, file6, indent=4)
    showinfo(message=lang[14])
# help & quit
def help(sysTrayIcon): os.system('README.txt')
def bye(sysTrayIcon): print("Bye")
# menu options
menu_options = ((lang[0], next(icons), select_files), # select files
                (lang[11], next(icons), ((lang[12], next(icons), next_wall), # wallpaper
                                        (lang[13], next(icons), per_wall))),
                (lang[1], next(icons), ((lang[2], next(icons), eng), # language
                                        (lang[3], next(icons), rus),
                                        (lang[4], next(icons), uzb))),
                (lang[6], next(icons), help)) # help
# show tray
